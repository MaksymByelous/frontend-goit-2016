...
var data = [
  ...
  $('body').append("<h1>SKILLS, UNIQ:<h1/>");
  for (var i in skills) {
    $('body').append(skills[i] + ", ");
  }

  var names = _.map(_.orderBy(data, 'friends.length'), 'name');
  var quantityFriends = [];
  for (var i = 0; i < names.length; i++) {
    for (var l= 0; l < data.length; l++) {
      if (names[i] == data[l].name) {
        quantityFriends[i] = data[l].friends.length;
      }
    }
  }
  $('body').append("<h1>NAMES, BY FRIENDS QUANTITY:<h1/>");
  for (var i in names) {
    $('body').append(names[i] + ": " + quantityFriends[i] + ", ");
  }

  var friends = _.uniq(_.map(_.flatten(_.map(data, 'friends')),'name'));
  $('body').append("<h1>FRIENDS NAMES:<h1/>");
